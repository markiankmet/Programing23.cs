...
namespace Task7_Programing.Controllers
{
    [ApiController]
    [Route("api/[controller]")]
    public class UsersController: Controller
    {
        readonly UsersAPIDbContext dbContext;
        ...
        public IActionResult GetCars(string? sortField)
        {
            if (sortField != null)
            {
                ...
                if (field != null)
                {
                    ...
                    {
                        var sortedList = dbContext.Cars.ToList().Where(x => x.State == "Published").OrderBy(x => x.GetType().
